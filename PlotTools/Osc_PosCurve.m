...
right_Fs500= round(right*500/Fs);
left_Fs500= round(left*500/Fs);
...
for row=1:length(right(:,1))
    for col=1:length(right)
        if right_Fs500(row,col)==0
            ...
            right_v(row,col)= posData(maxmin(2,row)+right_Fs500(row,col));
        end
        if left_Fs500(row,col)==0
            left_v(row,col)=0;
        else
            left_v(row,col)= posData(maxmin(7,row)+left_Fs500(row,col));
        end
    end
end
...
left_deg=left_v*-97.146+207.5025;
right_deg=right_v*-97.146+207.5025;
