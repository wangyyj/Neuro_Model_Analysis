...
right(right==0)=1;
left(left==0)=1;
psth_right=zeros(trials/4, max(max(right)));
for row_n=1:trials/phase
    for col_n=1:length(right(1,:))
        psth_right(row_n, right(row_n,col_n))=psth_right(row_n, right(row_n,col_n))+1;
    end
end

psth_left=zeros(trials/4, max(max(left)));
for row_n=1:trials/phase
    for col_n=1:length(left(1,:))
        psth_left(row_n, left(row_n,col_n))= psth_left(row_n, left(row_n,col_n))+1;
    end
end
