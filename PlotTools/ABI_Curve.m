% For loading ABI Data and plotting curves 11/5/10
% Modified from Freq_Curve

clear all;
%Query for file to be analyzed, load data
[FileName,PathName] = uigetfile('*.dat','cd');
full_filename=[PathName FileName];
[data, datainfo] = readHPData(full_filename);
rep=datainfo.curve.nreps;
trials=datainfo.curve.nTrials;  

% Demultiplex data (4 channels recorded, only chan2 is real data)
...
%sort spikecount according to Freq
...
%plot ABI Tuning curve
figure('Color',[1 1 1])
ABI = datainfo.curve.ABIrange;
errorbar(ABI,mean(spikecount_sorted2),std(spikecount_sorted2)/sqrt(rep)); % standard error of the mean
title('ABI Tuning');
xlabel('ABI (dB)');
ylabel('# Spike');